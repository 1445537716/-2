...
export default {
  ...
  methods: {
    ...
    async saveUser() {
      ...
      // 找到upload为false的则是还未上传成功
      if (fileList.some(item => !item.upload)) {
        this.$message.warning('图片还未上传完成')
        return
      }
      // 如果没有图片则上传空字符串 记住要打上空格 接口原因
    ...
    async savePersonal() {
      ...
      await updatePersonal({ ...this.formData, id: this.userId, staffPhoto: fileList ? fileList[0].url : ' ' })
      this.$message.success('保存成功')
    }
  }
}
